...
public class GUIRenderSystem extends IteratingSystem{
	...
	public GUIRenderSystem(final Batch batch, final TextureManager textureManager,final OrthographicCamera cam,int priority){
		super(Family.all(Wallet.class).get(),priority);
		this.batch = batch;
		this.textureManager = textureManager;
		this.camera= cam;
		//this.frame = new TextureRegion(textureManager.nameMap.get(TextureName.));
	}
	...
	protected void processEntity(Entity entity, float deltaTime) {
		...
		int greencount = wallet.greenCurrency;
		
		//batch.draw(textureManager., x, y, originX, originY, width, height, scaleX, scaleY, rotation);
	}
}
